...
const getAllItems = async (req, res) => {
  try {
    ...
    // Exclude claimed items from the list
    query.status = { $ne: 'Claimed', ...(query.status && { $eq: query.status }) };
...
const updateItemStatus = async (req, res) => {
  try {
    ...
    ).populate('reporter', 'name email').populate('claimant', 'name email');
...
const claimItem = async (req, res) => {
  try {
    const { id } = req.params;
    logger.info(`Claiming item ${id} by user ${req.user.id}`);
    ...
    if (item.status !== 'Found') {
      logger.warn(`Item ${id} status is ${item.status}, cannot claim`);
      return res.status(400).json({ error: 'Only found items can be claimed' });
    }

    if (item.claimant) {
      logger.warn(`Item ${id} already claimed by ${item.claimant}`);
      return res.status(400).json({ error: 'Item has already been claimed' });
    }

    if (item.reporter.toString() === req.user.id) {
      logger.warn(`User ${req.user.id} trying to claim their own item ${id}`);
    ...
    item.status = 'Claimed';
    await item.save();
    logger.info(`Item ${id} claimed successfully by user ${req.user.id}`);
    await item.populate('reporter', 'name email');
