...
const validateAndNormalizeEmail = (email) => {
  ...
  // Allow admin email or @chitkara.edu.in domain
  const adminEmail = (process.env.ADMIN_EMAIL || 'admin@campusconnect.com').toLowerCase();
  console.log('Validating email:', trimmedEmail, 'Admin email:', adminEmail);
  if (trimmedEmail !== adminEmail && !trimmedEmail.endsWith('@chitkara.edu.in')) {
...
const signIn = async (req, res) => {
  ...
  try {
    ...
    console.log('User found:', !!user, user ? { email: user.email, role: user.role, isVerified: user.isVerified } : 'null');
    if (!user) {
