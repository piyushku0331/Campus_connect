...
const getEvents = async (req, res) => {
  try {
    ...
    if (userRole !== 'admin') {
      ...
      countQuery = countQuery.where('isApproved', true);
    }
...
const getEventById = async (req, res) => {
  try {
    ...
    const event = await Event.findById(id)
      .populate('organizer_id', 'name avatar_url')
      .populate('attendees', 'name avatar_url');
...
const createEvent = async (req, res) => {
  try {
    ...
    const event = new Event({
      ...
      campus,
      category,
      isApproved: false // Events need admin approval
    });
